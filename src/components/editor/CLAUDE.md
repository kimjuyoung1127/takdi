...
## Files
- `node-editor-shell.tsx` — 3단 패널 쉘 + 키보드 단축키 + 자동 저장 + 토스트 알림 + 프로젝트 이름 인라인 편집 + mode prop 전달
- `floating-toolbar.tsx` — 플로팅 액션 바 (한글 툴팁, 파이프라인 단계 표시, 마지막 저장 시각, mode 인식 — 이미지 전용 모드에서 미리보기 숨김)
- `node-palette.tsx` — 좌측 작업 단계 팔레트 (모드별 노드 필터링, MODE_NODE_CONFIG 기반)
- `node-canvas.tsx` — React Flow 캔버스 (Delete 키, 빈 캔버스 온보딩, 우클릭 메뉴, 미니맵, Undo/Redo, 엣지 애니메이션)
- `takdi-node.tsx` — 커스텀 React Flow 노드 (아이콘 + 라벨 + StatusBadge + Handle)
- `properties-panel.tsx` — 우측 설정 패널 (설정/파일/기록/비용 탭, 비선택 시 프로젝트 요약 + 단축키 가이드, 상태 한글 표시)
...
- 노드 타입 키: `prompt` (구 `generate`), `generate-images`, `bgm`, `cuts`, `render`, `export`
