...
export function ImageUploadZone({
  ...
  if (showAssets) {
    return (
      ...
      >
        <div className="flex items-center justify-between border-b border-[#EEE6DC] px-3 py-2">
          <span className={`text-xs font-medium ${WORKSPACE_TEXT.body}`}>프로젝트 파일</span>
          <button
            onClick={() => setShowAssets(false)}
            className={`text-xs ${WORKSPACE_TEXT.muted} hover:text-[#4D433D]`}
          >
            닫기
          </button>
        </div>
  ...
  return (
    ...
    >
      ...
      {imageUrl ? (
        ...
        <div className="flex h-full min-h-[120px] flex-col items-center justify-center border-2 border-dashed border-[#D5CCC3] bg-[#F8F4EF] transition-colors hover:border-[#D97C67] hover:bg-[#F8E7E2]/50">
          {uploading ? (
            ...
            <>
              <button
                onClick={handleClick}
                className={`flex flex-col items-center gap-2 rounded-2xl px-5 py-3 transition-colors ${WORKSPACE_CONTROL.accentTint} hover:bg-[#F6DFD8]`}
              >
              ...
              <button
                onClick={(e) => { e.stopPropagation(); setShowAssets(true); }}
                className="mt-2 flex items-center gap-1 rounded-2xl border border-[#E5DDD3] bg-white px-2.5 py-1.5 text-[11px] text-[#6F655D] hover:bg-[#F8F4EF] hover:text-[#201A17]"
              >
