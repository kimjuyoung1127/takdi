  ...
  return (
    ...
    >
      ...
      {block.overlays.map((overlay) => (
        <div
          key={overlay.id}
          className={`absolute ${readOnly ? "select-none" : "cursor-move select-none"} ${selected && !readOnly ? "rounded border border-dashed border-white/50 px-1 hover:border-white/80" : ""}`}
          style={{
            left: `${overlay.x}%`,
            top: `${overlay.y}%`,
