...
export function RemotionPlayerRuntime({
  ...
  if (status !== "ready" || !PlayerComponent || !CompositionComponent) {
    return (
      <div
        className={cn(
          "mx-auto flex w-full flex-col items-center justify-center rounded-3xl border border-slate-200 bg-slate-50 p-6 text-center",
          composition.maxWidthClassName,
        )}
      >
        <div
          className={cn(
            "flex w-full items-center justify-center overflow-hidden rounded-2xl border border-dashed border-slate-300 bg-white",
            composition.aspectClassName,
          )}
        >
          <div className="space-y-3 px-6 text-slate-600">
            {status === "error" ? (
              <>
              ...
              <>
                ...
                <p className="text-sm font-semibold">Loading preview runtime</p>
